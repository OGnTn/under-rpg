...
@onready var reference_node: Node3D = parent.get_parent().get_parent()

func _ready() -> void:
	if not parent or not reference_node:
		push_error("ArmSpring script requires a parent Node3D and a reference Node3D (ViewModel)!")
		set_process(false)
		return
		
	# Initialize spring state to match the parent's transform in reference_node's local space
	var target_transform = reference_node.global_transform.affine_inverse() * parent.global_transform
	current_pos = target_transform.origin
	current_rot = target_transform.basis.get_rotation_quaternion()
...
func _process(delta: float) -> void:
	...
	if displacement.length() > 5.0:
		...
		angular_vel = Vector3.ZERO
		global_transform = reference_node.global_transform * Transform3D(Basis(current_rot), current_pos)
		return
